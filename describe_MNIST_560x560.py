...
def process_image(rank, image, label, model_id, hf_token, debug=False):
    try:
        model = MllamaForConditionalGeneration.from_pretrained(
            model_id, torch_dtype=torch.bfloat16, device_map=f"cuda:{rank}", token=hf_token, local_files_only=True
        )
        processor = AutoProcessor.from_pretrained(model_id, token=hf_token, local_files_only=True)
        
        # Convert to RGB and resize to 560x560
        if not isinstance(image, Image.Image):
            image = Image.fromarray(image.numpy(), mode='L')
        image = image.convert('RGB').resize((560, 560), Image.Resampling.LANCZOS)
